...
Dos excepciones puntuales a esta neutralidad: `GET /api/cuentas/{id}/mobile` (pensado específicamente para reducir el tráfico hacia BFF Mobile) y `GET /api/transacciones/recientes` / `GET /api/movimientos/recientes` (sin restricción de rol, pensados para que un `CLIENTE` pueda consultar su propio resumen, no solo un `EMPLEADO`). Ambos se documentan como trade-offs deliberados en el informe técnico.
...
| **Web** | Completo (todos los campos de la cuenta, más un dashboard con transacciones y movimientos recientes agregados en una sola respuesta) | Sesión en un entorno más controlado y privado; al ser el canal de mayor capacidad de despliegue de información, agrega y consolida datos de múltiples fuentes para ofrecer una vista de resumen completa, evitando que el cliente web deba hacer múltiples llamadas por separado |
...
`GET /bff-web/cuentas/{id}/dashboard` combina, en una sola respuesta, el detalle de la cuenta junto con sus 5 transacciones y 5 movimientos más recientes — demuestra el patrón de **agregación** propio de un BFF: el cliente web recibe todo lo necesario para una vista de resumen en una sola llamada, en vez de tener que hacer 3 peticiones separadas. Accesible tanto para `CLIENTE` (el usuario final, dueño de la cuenta) como para `EMPLEADO`; la cuenta consultada sigue validando propiedad, mientras que las transacciones y movimientos recientes se muestran como resumen general del sistema (ver limitaciones).

Las tres fuentes se consultan **en paralelo** mediante `CompletableFuture`, ya que son independientes entre sí — el tiempo de respuesta queda determinado por la llamada más lenta, no por la suma de las tres. Evidencia registrada en consola muestra las tres tareas iniciándose con 1-2 ms de diferencia entre sí, cada una en su propio hilo, con una mejora de tiempo de respuesta de aproximadamente 3 veces frente a una ejecución secuencial (ver detalle en el informe técnico).
...
| `403` | Rol insuficiente para la operación, o cuenta consultada que no pertenece al usuario autenticado (incluye el dashboard) | `"No tienes permisos para acceder a esta cuenta"` |
...
| Servicio | Puerto | URL base | Endpoints destacados |
|---|---|---|---|
| **Backend Central** | `8081` | `http://localhost:8081/api` | `/auth/login`, `/cuentas/{id}`, `/cuentas/{id}/mobile`, `/transacciones/recientes`, `/movimientos/recientes` |
| **BFF Cajeros** | `8082` | `http://localhost:8082/bff-cajero` | `/cuentas/{id}/saldo` |
| **BFF Mobile** | `8083` | `http://localhost:8083/bff-mobile` | `/cuentas/{id}/saldo` |
| **BFF Web** | `8084` | `http://localhost:8084/bff-web` | `/cuentas/{id}`, `/cuentas/{id}/dashboard` |
...
- `transacciones` y `movimientos_anuales` no tienen una relación confiable con una cuenta específica en el dataset de origen; por ello, `GET /api/transacciones/recientes` y `/movimientos/recientes` (usados en el dashboard) muestran un resumen general del sistema, sin filtrar por cuenta, y están disponibles para cualquier usuario autenticado.
